...
class RabinKeyManagement:
    def __init__(self, key_size=1024):
        ...
        self.keys = {}  # Store keys for facilities
    ...
    def encrypt(self, public_key, plaintext):
        ...
        ciphertext = pow(m, 2, n)  # Rabin encryption: c = m^2 mod n
        pt_hash = hashlib.sha256(plaintext.encode()).digest()  # hash for disambiguation
        return (
    ...
    def decrypt(self, private_key, ciphertext, pt_hash):
        ...
        # Compute four square roots modulo p and q
        root_p1 = pow(c, (p + 1) // 4, p)
        root_p2 = (-root_p1) % p
        root_q1 = pow(c, (q + 1) // 4, q)
        root_q2 = (-root_q1) % q

        # CRT to combine roots
        def chinese_remainder_theorem(a1, a2, n1, n2):
            m1 = inverse(n1, n2)
            return (a1 + (a2 - a1) * m1 * n1) % (n1 * n2)

        roots = [
            chinese_remainder_theorem(root_p1, root_q1, p, q),
            chinese_remainder_theorem(root_p1, root_q2, p, q),
            chinese_remainder_theorem(root_p2, root_q1, p, q),
            chinese_remainder_theorem(root_p2, root_q2, p, q)
        ]

        # Select the correct plaintext using the hash
        for plaintext in roots:
